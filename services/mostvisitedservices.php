    ...
    or die('Could not connect: ' . pg_last_error());
      ...
      //  mysql_select_db($db_name);
        $searchquery="select * from services order by visit desc LIMIT 4;";
        ...
        while ($row = pg_fetch_array($sqlsearch,null, PGSQL_ASSOC)) {
                ...
                $arr[$i]=array($row["id"], $row["title"],$row["page_url"],$row["image_url"],$row["description"],$row["total_rating"],$row["num_rating"],$row["visit"],$row["costs"]);
                $i++;
        }
